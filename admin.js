                ...
                pdf.getPage(i).then(page => {
                    page.getTextContent().then(textContent => {
                        ...
                        if (studentMatch) {
                            ...
                            const id = studentMatch[2].trim();
                            const grades = text.match(/(\w+\s*\w*):\s*(\d+)/gi)?.map(g => {
                                const [subject, grade] = g.split(':').map(s => s.trim());
                                return [subject, parseInt(grade) || 0];
                            }) || [];
                            const results = Object.fromEntries(grades);
                            const total = grades.reduce((sum, [, grade]) => sum + grade, 0);
                            const average = grades.length ? (total / grades.length).toFixed(2) : 0;
                            const status = average >= 50 ? 'ناجح' : 'راسب';
                            allResults.push({ name, id, results, average, status });

                            // تحديث أو إضافة الطالب في localStorage
                            let student = students.find(s => s.id === id);
                            if (student) {
                                student.subjects = grades.map(([name, grade]) => ({ name, grade }));
                            } else {
                                const username = generateUniqueUsername(name, id);
                                ...
                                student = {
                                    ...
                                    username,
                                    password: hashedPassword,
                                    subjects: grades.map(([name, grade]) => ({ name, grade })),
                                    profile: { email: '', phone: '', birthdate: '', address: '', bio: '' },
                                    originalPassword
                                };
                        ...
                        if (i === numPages) {
                            if (saveToLocalStorage('students', students)) {
                                displayPDFResults(allResults);
                                renderResults(); // تضاف النتائج لقائمة النتائج
                                renderStats();
                                showToast(`تم تحليل الملف وإضافة/تحديث ${allResults.length} طالب بنجاح!`, 'success');
                            } else {
                                showToast('حدث خطأ أثناء حفظ البيانات!', 'error');
                            }
                        }
                    }).catch(err => {
                        showToast('خطأ أثناء قراءة صفحة الملف!', 'error');
                        console.error(err);
                    });
                }).catch(err => {
                    showToast('خطأ أثناء تحميل صفحة الملف!', 'error');
                    console.error(err);
                });
            }
        }).catch(err => {
            showToast('خطأ أثناء تحميل الملف، تأكد من أنه PDF صالح!', 'error');
            console.error(err);
        });
    };
...
function displayPDFResults(results) {
    ...
    results.forEach(student => {
        ...
        const percentage = student.results.length ? (total / (student.results.length * 100)) * 100 : 0;
        const row = document.createElement('tr');
        row.innerHTML = `
